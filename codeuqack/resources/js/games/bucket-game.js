...
function startLevel(lessonOrder, lessonTitle) {
  ...
  if (levelTitleEl) {
    levelTitleEl.textContent = `Level ${lessonOrder} – ${lessonTitle}`;
  }
  ...
  if (!questions.length) {
    // No bucket-game questions defined yet for this level.
    codeLineEl.textContent = "Coming soon for this level!";
    bucketsRow.innerHTML   = "";
    tokenPool.innerHTML    = "";
    placedCount.textContent = "0 / 0 placed";
    qNum.textContent   = 0;
...
async function endGame() {
    ...
    try {
        ...
        const completed =
            data.progress?.[course]?.bucketLevelsCompleted || [];
        ...
        if (firstCompletion) {
            ...
            await updateDoc(ref, {
                ...
                [`progress.${course}.bucketLevelsCompleted`]:
                    arrayUnion(currentLevel)
